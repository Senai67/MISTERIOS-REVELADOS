...
const MasterChat = ({ isOpen, onClose, onNavigateToBook }) => {
    const [messages, setMessages] = useState([
        { role: 'master', text: 'Bienvenido, buscador de la verdad. Escribe cualquier palabra o frase y buscaré su rastro en los manuscritos de Saint Germain.' }
    ]);
    ...
    const handleJumpToResult = (result) => {
        // Find the real book data from libros.js
        // result.volumeId is 1-indexed, book.id is 0-indexed
        const bookIndex = parseInt(result.volumeId) - 1;
        const book = libros[bookIndex];

        if (book && onNavigateToBook) {
            // Find the chapter index by matching the chapter title
            const chapterIndex = book.capitulos.findIndex(
                cap => cap.titulo === result.chapterTitle
            );

            if (chapterIndex !== -1) {
                // Navigate to the book and chapter
                onNavigateToBook(book, chapterIndex);
            } else {
                // Fallback: navigate to book without specific chapter
                onNavigateToBook(book, 0);
            }
        }
    };
            ...
            <div className="w-full max-w-2xl bg-parchment shadow-mystic rounded-lg overflow-hidden flex flex-col h-[80vh] border-2 border-gold/30 paper-edge">
                ...
                <div className="bg-parchment-dark/80 px-6 py-4 flex justify-between items-center border-b border-gold/20">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h3 className="font-serif text-gold-dim leading-none">Buscador de Manuscritos</h3>
                            <span className="text-[10px] uppercase tracking-widest text-ink/50">Saint Germain</span>
                        </div>
                    </div>
                                            ...
                                            {msg.results.map((res, resIdx) => (
                                                ...
                                                >
                                                    <div className="flex justify-between items-start mb-2">
                                                        <span className="text-xs font-sans text-gold-dim">
                                                            Libro {res.volumeId} • {res.chapterTitle}
                                                        </span>
                                                        <span
                ...
                <div className="p-4 bg-parchment-light border-t border-gold/20">
                    <div className="relative flex items-center">
                        <input
                            ...
                            placeholder="Busca enseñanzas, conceptos, lugares..."
                            className="w-full bg-transparent border-b-2 border-gold/10 focus:border-gold py-3 px-4 pr-12 outline-none font-sans text-ink placeholder:text-ink/50 transition-all"
                        />
                        <button
                            onClick={handleSend}
